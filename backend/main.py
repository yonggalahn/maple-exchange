...
def get_apartment_comment(key: str, pyeong: float) -> str:
    comments = APARTMENT_SPECS[key]["comments"]
    for limit, comment in comments:
        if pyeong < limit:
            return comment
    return comments[-1][1]

def get_car_comment(key: str, qty: float) -> str:
    comments = CAR_SPECS[key]["comments"]
    for limit, comment in comments:
        if qty < limit:
            if limit == float('inf'):
...
def get_tier_info(meso: int) -> tuple[str, str]:
    if meso < 10000000:
...
def exchange_meso_get(meso: int = Query(..., description="환전할 메이플랜드 메소 금액")):
    if meso < 0:
    ...
    for key, spec in APARTMENT_SPECS.items():
        ...
        m2 = pyeong * 3.30578
        comment = get_apartment_comment(key, pyeong)
    ...
    for key, spec in CAR_SPECS.items():
        ...
        qty = krw / car_price
        comment = get_car_comment(key, qty)
    ...
    tier_name, tier_desc = get_tier_info(meso)
...
def exchange_meso_post(request: ExchangeRequest):
    return exchange_meso_get(request.meso)
